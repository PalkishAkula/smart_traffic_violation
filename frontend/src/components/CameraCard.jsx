...
export default function CameraCard({ camera, onDelete }) {
  ...
  const handleStart = async () => {
    try {
      await startCamera(camera.camera_id);
    } catch (err) {
      alert(t("Failed to start camera"));
    }
  };

  const handleStop = async () => {
    try {
      await stopCamera(camera.camera_id);
    } catch (err) {
      alert(t("Failed to stop camera"));
    }
  };
        ...
        <div className="aspect-video bg-gray-900 relative flex items-center justify-center group">
          {isRunning && frameUrl ? (
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-gray-500">{t("No live feed")}</p>
            </div>
          )}
          ...
          <div className="absolute top-3 right-3">
            ...
            >
              <span
                className={`w-1.5 h-1.5 rounded-full ${isRunning ? "bg-white animate-pulse" : "bg-gray-300"}`}
              />
          ...
          >
            ...
            <div className="mt-3 flex items-center justify-between">
              <p className="text-xs text-gray-500">
                {t("Source")}: {camera.source || t("Unknown")}
              </p>
              <p className="text-xs text-gray-500">
                Press{" "}
                <kbd className="px-1.5 py-0.5 bg-gray-800 text-gray-300 rounded text-[10px] font-mono">
                  ESC
                </kbd>{" "}
