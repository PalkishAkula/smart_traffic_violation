...
import socket, { connectSocket, subscribeCamera, unsubscribeCamera } from '../services/socket'

export default function Cameras() {
    const { cameras, fetchCameras, addCamera, deleteCamera, updateFrame,
        updateCameraStatus, loading } = useCameraStore()
        ...
        <div className="space-y-6">
            ...
            {loading ? (
                ...
                <div className="bg-white rounded-2xl border border-gray-200 p-16 text-center">
                    ...
                    <h3 className="text-sm font-semibold text-gray-700 mb-1">No cameras added</h3>
                    <p className="text-xs text-gray-400 mb-4">Add a camera to start live detection</p>
                    <button
                        onClick={() => setShowModal(true)}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {cameras.map(cam => (
                        <CameraCard key={cam.camera_id} camera={cam} onDelete={handleDelete} />
                    ))}
                </div>
            )}
                ...
                <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
                    <div className="bg-white rounded-2xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between mb-5">
                            <h3 className="text-lg font-bold text-gray-900">Add Camera</h3>
                        ...
                        <form onSubmit={handleAdd} className="space-y-4">
                            <div>
                                <label className="text-xs font-semibold text-gray-600 mb-1.5 block">Camera ID</label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="text-xs font-semibold text-gray-600 mb-1.5 block">Source</label>
                                <input
                                    type="text"
                                    value={form.source}
                                    ...
                                    required
                                />
                                <p className="text-xs text-gray-400 mt-1">Enter a number for webcam index or a URL for IP camera</p>
                            </div>
                            <button
                                type="submit"
